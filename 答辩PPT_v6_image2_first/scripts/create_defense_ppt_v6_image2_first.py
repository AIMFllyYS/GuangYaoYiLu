...
OUT = ROOT / "答辩PPT_v6_image2_first"
V4 = ROOT / "答辩PPT_v4"
...
def build_layers_for_slide(spec: SlideSpec, logo_path: Path, yaoguang_mark: Image.Image) -> tuple[list[Layer], Path]:
    ...
    if spec.slide <= 7:
        ...
        add_layer(layers, spec, yaoguang_mark.resize((132, 132), Image.Resampling.LANCZOS), "yaoguang-slot", "yaoguang-slot", "image", "yaoguang_character", 1648, 780, 26, "yaoguang_character", "瑶光文创/瑶光-3.jpg", "Independent yaoguang visual clue layer.")
        if spec.slide == 4:
            add_layer(layers, spec, make_merch_overlay(), "merch-product-base", "merch-product-base", "image", "generated_decoration", 120, 300, 24, "merch-product-base", "瑶光文创", "Product shapes are schematic references only, not copied brand graphics.")
        if spec.slide == 5:
